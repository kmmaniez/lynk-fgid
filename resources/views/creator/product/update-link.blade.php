    ...
    <section id="create-link" class="">
        <div class="card border-0">
            <div class="card-body">
                ...
                <form id="form" action="{{ route('products.linkupdate', $product->id) }}" method="post"
                    ...
                    <div class="form-group hstack gap-3 justify-content-between align-items-center">
                        @if ($product->thumbnail)
                            <div class="box position-relative rounded-2">
                                <img id="displaythumbnail" src="{{ Storage::url($product->thumbnail) }}"
                        ...
                        @else
                            <div class="box position-relative rounded-2" style="display: none">
                                <img id="displaythumbnail" class="position-relative rounded-1"
                        ...
                        <div class="form-input vstack justify-content-around w-100">
                            <input type="text" class="form-control" value="{{ $product->name }}" name="name"
                                id="name">
                            <input type="url" class="form-control" value="{{ $product->url }}" name="url"
                                id="url">
                        </div>
                    </div>
                    ...
                    <button class="btn btn-md bg-danger bg-gradient text-white fw-semibold text-uppercase w-100 mt-2">Update
                        Link</button>
                </form>
    ...
    <script>
        ...
        $('#removeimg').on('click', (e) => {
            ...
            if (confirm("Are you sure you want to delete this?")) {
                ...
                $.ajax({
                    url: '/creator/delete-image-link/' + '{{ $product->id }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        user_id: '{{ $user->id }}'
